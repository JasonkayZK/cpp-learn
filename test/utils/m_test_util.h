...
template <typename T> struct calculate_time_t {
  T _data;
  double _time_ms;
  double _time_s;

  calculate_time_t(const T &data, const double time_ms,
                   const double time_s) noexcept
      : _data(data), _time_ms(time_ms), _time_s(time_s){};
};
...
auto calculate_time(Fn &fn, Args &&...args) {
  ...
  return calculate_time_t<decltype(0)>{
      0, static_cast<double>(t2) - static_cast<double>(t1),
...
auto calculate_time_with_return(Fn &fn, Args &&...args) {
  ...
  return calculate_time_t<decltype(_tmp)>(
      _tmp, static_cast<double>(t2) - static_cast<double>(t1),
      (static_cast<double>(t2) - static_cast<double>(t1)) / CLOCKS_PER_SEC);
}
